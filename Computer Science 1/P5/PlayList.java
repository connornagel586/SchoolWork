...
public class PlayList implements PlayListInterface {
	...
	/**
	 * @param removes
	 *            song from playlist in location 'id'
	 */
...
public int moveUp(int index){
	...
	public Song[][] getMusicSquare() {
		int size = (int) Math.ceil(Math.sqrt(songList.size()));
		Song[][] MusicSquare = new Song[size][size];

		for (int i = 0; i < size; i++) {
			for (int j = 0; j < size; j++) {

				int index = (i * MusicSquare.length + j) % songList.size();

				MusicSquare[i][j] = songList.get(index);
			}
		}
		return MusicSquare;
	}
