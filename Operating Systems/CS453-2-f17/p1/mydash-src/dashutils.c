...
int backgroundCheck(char *line){
	...
	while((char) bufLine[x] == ' ' && x < 0){ //Might need adjustment
		bufLine[x] = '\0';
			x--;
	}
	if((char)bufLine[x] == '&'){
...
char *toString(const void *obj){
	...
	int thisDone = thisObject->done;
	char *thisProcess = (char *)thisObject->process;
	char *thisString;
	if(thisDone){
	thisString = (char *)malloc((sizeof(char) * 4) + 14);
       	sprintf(thisString ,"[%d] Done %s &", thisNum, thisProcess);
