...
struct tsb_list * tsb_createList(int (*equals)(const void *, const void *), 
	...
	struct tsb_list *tsList = (struct tsb_list*) malloc(sizeof(struct tsb_list*) + capacity * sizeof(struct node));

	tsList->list = createList(equals, toString, freeObject);
	tsList->capacity = capacity;
	tsList->stop_requested = 0;
	pthread_mutex_init(&(tsList->mutex), NULL);
	pthread_cond_init(&(tsList->listNotFull),NULL);
	pthread_cond_init(&(tsList->listNotEmpty), NULL);
...
NodePtr tsb_removeNode(struct tsb_list *list, NodePtr node){
	...
	NodePtr removnode = NULL;
	if(list->stop_requested == 0){
		removnode = removeNode(list->list, node);
	}
	pthread_mutex_unlock(&(list->mutex));
